...
type Config struct {
	...
	// Theme names the palette keyforge paints with: a theme name, or a path to a palette file.
	// Empty means whatever lvim-colorscheme currently says is active, and the built-in Everforest
	// when that resolves to nothing — see internal/theme for the order and why it is that order.
...
func Path() string { return configFile("config.json", "keyforge.json") }

// ThemeDir is where keyforge looks for palettes, beside the config it belongs with. One directory
// of small documents rather than colours inside config.json: a theme is generated by another tool
// and dropped in, and generated files have no business being merged into hand-edited settings.
func ThemeDir() string { return configFile("themes", "keyforge-themes") }

// configFile resolves one file in keyforge's config directory.
//
// Split out of Path because the config is no longer the only thing keyforge keeps there — see
// TreeStatePath — and two copies of the XDG lookup are two places for a machine that sets
// XDG_CONFIG_HOME to be honoured by one of them and not the other. The fallback is per file
// rather than derived from the name, so an existing installation's `keyforge.json` keeps being
// found on the one machine that has no home directory to speak of.
func configFile(name, fallback string) string {
	if d := os.Getenv("XDG_CONFIG_HOME"); d != "" {
		return filepath.Join(d, "keyforge", name)
	}
	home, err := os.UserHomeDir()
	if err != nil {
		return fallback
	}
	return filepath.Join(home, ".config", "keyforge", name)
}
